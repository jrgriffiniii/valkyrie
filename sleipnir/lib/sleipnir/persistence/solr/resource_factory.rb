...
module Sleipnir::Persistence::Solr
  class ResourceFactory
    require 'sleipnir/persistence/solr/mapper'
    ...
    def from_model(model)
      Hash[::Sleipnir::Persistence::Solr::Mapper.find(model).to_h.merge(internal_model_ssim: model.internal_model).merge(indexer_solr(model))]
    end
    ...
    class ModelBuilder
      ...
      class SolrValue < ::Sleipnir::ValueMapper
      end
      class LanguagePropertyValue < ::Sleipnir::ValueMapper
      ...
      class PropertyValue < ::Sleipnir::ValueMapper
        SolrValue.register(self)
        def self.handles?(value)
          value.is_a?(Property)
        end
      ...
      class IntegerValue < ::Sleipnir::ValueMapper
        SolrValue.register(self)
        def self.handles?(value)
