...
RSpec.shared_examples 'a Sleipnir::Persister' do |*flags|
  before do
    raise 'persister must be set with `let(:persister)`' unless defined? persister
    class CustomResource < Sleipnir::Model
  ...
  it "can store Sleipnir::Ids" do
    shared_title = persister.save(model: resource_class.new(id: "test"))
    book = persister.save(model: resource_class.new(title: [shared_title.id, Sleipnir::ID.new("adapter://1"), "test"]))
    reloaded = query_service.find_by(id: book.id)
    expect(reloaded.title).to contain_exactly(shared_title.id, Sleipnir::ID.new("adapter://1"), "test")
    expect([shared_title.id, Sleipnir::ID.new("adapter://1"), "test"]).to contain_exactly(*reloaded.title)
  end
  ...
  it "can delete objects" do
    ...
    expect { query_service.find_by(id: persisted.id) }.to raise_error ::Sleipnir::Persistence::ObjectNotFoundError
  end

  context "when wrapped with a form object" do
    let(:myclass) { Class.new(Sleipnir::Form) { self.fields = [:title, :member_ids] } }
